...
def load_or_create_config():
    ...
    default_config["PASSWORD_HASH"] = hashlib.sha256(pw.encode()).hexdigest()
...
class UniversalHandler(http.server.SimpleHTTPRequestHandler):
    def check_auth(self):
        """Verifica si el usuario tiene la cookie de acceso o un token válido."""
        # 1. Verificar token en la URL (para visores como Google Docs)
        parsed_path = urllib.parse.urlparse(self.path)
        query = urllib.parse.parse_qs(parsed_path.query)
        
        # VALIDAR HASH EN LUGAR DE TEXTO PLANO
        token = query.get('token', [None])[0]
        # Para compatibilidad, si el token viene en texto plano, lo hasheamos para comparar
        # O idealmente, el cliente debería enviar el hash. 
        # Dado que Google Docs/Zoho no hashean, asumimos que token es la password plana en este caso específico?
        # NO, por seguridad, cambiemos a que el token URL debe ser el hash.
        ...
        if cookie_header:
            ...
            if 'access_token' in cookie and cookie['access_token'].value == PASSWORD_HASH:
                return True
        
        # Si es un POST de login, lo procesamos
        if self.path == "/login" and self.command == "POST":
            return True # Permitir el POST para procesarlo en do_POST
            ...
            <div class="card" style="padding:40px; text-align:center; width:90%; max-width:400px;">
                ...
                <form action="/login" method="POST">
                    <input type="password" name="pw" placeholder="Introduce la contraseña" 
                           style="width:100%; padding:15px; border-radius:8px; border:1px solid var(--border); margin-bottom:10px; font-size:16px; outline:none; background:var(--bg); color:var(--text);">
                    <div id="login-error" style="color: #ff4d4d; font-size: 14px; margin-bottom: 15px; display: none;">❌ Contraseña incorrecta</div>
    ...
    def do_POST(self):
        if self.path == "/login":
            ...
            # HASHEAR PASSWORD INGRESADA
            pw_hash = hashlib.sha256(pw.encode()).hexdigest()
            
            if pw_hash == PASSWORD_HASH:
                self.send_response(303)
                # Removido 'Secure' para que funcione en HTTP local (PC/Móvil en casa)
                self.send_header("Set-Cookie", f"access_token={PASSWORD_HASH}; Path=/; HttpOnly; SameSite=Lax; Max-Age=2592000")
...
def run_server():
    ...
    if NGROK_TOKEN:
        try:
            ...
            if NGROK_DOMAIN:
                public_url = ngrok.connect(port, domain=NGROK_DOMAIN, pyngrok_config=config).public_url
            else:
